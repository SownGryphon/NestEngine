...
int checkLine(in ivec2 pos1, in ivec2 pos2, bool lastContourSeg)
{
	...
	if (posDiff.y == 0 || crossVec.y < 0 || crossVec.y > abs(posDiff.y) || crossVec.x > 0)
	{
		return 0;
	}
	...
	if (lastContourSeg && crossVec.y == abs(posDiff.y) && result == firstCrossDir)
		return 0;

	if (firstCrossDir == 0 && crossVec.y == 0)
